...
function modelFixture(p: string, id: string, o?: { reasoning?: boolean }): Model {
	return buildModel({ id, provider: p, api: "openai-completions", name: `${p}/${id}`, baseUrl: "https://example.invalid", reasoning: o?.reasoning ?? false, input: ["text"] as ("text" | "image")[], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 16_000, maxTokens: 4_096 });
}
...
describe("auditStrictRoleBindings", () => {
	...
	it("reports valid for a subagent-capable role with exact available model", () => {
		...
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "smol")!;
	...
	it("marks unavailable when concrete selector has no matching model", () => {
		const s = Settings.isolated({ modelRoles: { smol: "openai-completions/gpt-4o" } });
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o-mini")])).find(x => x.roleId === "smol")!;
	...
	it("marks unconfigured when role has no modelRoles entry", () => {
		const s = Settings.isolated({ modelRoles: { smol: "openai-completions/gpt-4o" } });
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "vision")!;
	...
	it("marks custom role without contract as contractStatus incomplete", () => {
		const s = Settings.isolated({ modelRoles: { "custom:r": "openai-completions/gpt-4o" }, modelTags: { "custom:r": { name: "R" } } });
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "custom:r")!;
		expect(e.modelStatus).toBe("valid");
	...
	it("marks default (canRunAsSubagent=false) as contractStatus incomplete", () => {
		...
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "default")!;
		expect(e.modelStatus).toBe("valid");
		expect(e.contractStatus).toBe("incomplete");
		expect(e.executable).toBe(false);
	});
	...
	it("marks tiny (canRunAsSubagent=false) as contractStatus incomplete", () => {
		...
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "tiny")!;
		expect(e.modelStatus).toBe("valid");
		expect(e.contractStatus).toBe("incomplete");
		expect(e.executable).toBe(false);
	});
	...
	it("includes roles from modelRoles even without modelTags", () => {
		...
		const ids = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).map(x => x.roleId);
	...
	it("shapes each entry with stable keys including message", () => {
		const s = Settings.isolated({ modelRoles: { smol: "openai-completions/gpt-4o" } });
		const e = auditStrictRoleBindings(s, fakeRegistry([modelFixture("openai-completions", "gpt-4o")])).find(x => x.roleId === "smol")!;
		expect(Object.keys(e).sort()).toEqual(["contractStatus", "executable", "message", "modelStatus", "roleId", "selector"].sort());
		expect(typeof e.message).toBe("string");
	});
	...
	it("delegates to Task1 validateRoleContractForTask from role-contract-validator", () => {
		...
		const result = validateRoleContractForTask({ roleId: "default", roleInfo: info, requirements: { needsProductionWrite: false, needsTestWrite: false, readOnly: false } });
