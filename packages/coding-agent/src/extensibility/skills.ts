...
import type {
	SuperpowersCodebaseMemoryGateMode,
	SuperpowersCodebaseMemoryStatus,
} from "../superpowers/codebase-memory-gate";
import { resolveSuperpowersCodebaseMemoryGate } from "../superpowers/codebase-memory-gate";
...
export async function buildSkillPromptMessage(
	...
	args: string,
	invocationOrOptions: SkillInvocationKind | BuildSkillPromptMessageOptions = "user",
	optionsArg?: BuildSkillPromptMessageOptions,
): Promise<BuiltSkillPromptMessage> {
	const invocation = typeof invocationOrOptions === "string" ? invocationOrOptions : "user";
	const options = typeof invocationOrOptions === "string" ? optionsArg : invocationOrOptions;
	...
	const gate = resolveSuperpowersCodebaseMemoryGate({
		...
		userPrompt: message,
		enabled: options?.codebaseMemoryGate?.enabled,
		mode: options?.codebaseMemoryGate?.mode,
		status: options?.codebaseMemoryGate?.status,
	});
	if (gate.shouldInject && gate.contextMessage) {
		message = `${gate.contextMessage}\n\n${message}`;
	}
	return {
		message,
		details: {
