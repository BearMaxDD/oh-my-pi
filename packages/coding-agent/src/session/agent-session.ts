...
export class AgentSession {
		...
		try {
			...
			if (wantsSnapcompact && !this.model.input.includes("image")) {
				...
				if (!renderScan.isSafe) {
					...
					throw new Error("snapcompact cannot run locally: high non-ASCII rate detected.");
				}
			}
	...
	async #runAutoCompaction(
		...
		let routeReason: string | undefined =
			reason === "overflow" && compactionSettings.strategy === "snapcompact" ? "overflow_recovery" : undefined;
	...
	switchAdvisorModel(modelRoleValue: string, options: AdvisorModelSwitchOptions = {}): boolean {
		const previousModel = this.getAdvisorAgent()?.state.model;
		...
		const nextModel = this.getAdvisorAgent()?.state.model;
	...
	getAdvisorModelAssignment(): ModelAssignment | undefined {
		const model = this.getAdvisorAgent()?.state.model;
		if (!model) return undefined;
